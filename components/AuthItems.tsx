...
const AuthItems: FC = observer(() => {
  ...
  return (
    <div className={styles.items}>
      ...
      />
      {store.isAuth ?
        <button  onClick={() => store.signIn(state.email, state.password)} >Войти</button>
        :
        <button onClick={() => store.firstSignUp(state.firstName, state.secondName, state.nick, state.email, state.password)} >Создать</button>
      }
      {store.isAuth ?
